...
const SCRAPING_APP_URL =
  process.env.SCRAPING_APP_URL ?? "https://pdf-generator-api-z5z6k2buaq-de.a.run.app";
...
export const urlToMarkdownWithScraperService = async ({
  ...
  const processedResults = await Promise.all(
    results.map(async (result) => {
      ...
      try {
        const urlHash = crypto.createHash("sha256").update(result.url).digest("hex");
...
export const downloadPDFTask = async ({
  ...
  const promises = pdfSources.map(async (source) => {
    try {
      const response = await fetch(`${SCRAPING_APP_URL}/api/download-and-upload`, {
        method: "POST",
        headers: {
