...
export class ScraperService {
  ...
  private async fetch<T>(endpoint: string, body: Record<string, unknown>): Promise<T> {
    ...
    if (!response.ok) {
      ...
      logger.error(`Scraper service error: ${response.status} ${response.statusText}`, {
        error: errorText,
        endpoint,
      });
  ...
  async scrapePdfBulk(urls: string[]): Promise<ScraperResult[]> {
    ...
    try {
      const response = await this.fetch<BulkScrapeResponse<ScraperResult>>("/api/scrape/pdf/bulk", {
        urls,
      });
  ...
  async captureVisual(url: string, type: "pdf" | "screenshot"): Promise<ScraperResult> {
    try {
      const response = await this.fetch<{
        success: boolean;
        url: string;
