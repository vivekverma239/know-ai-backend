...
const getAllFilesTocInfo = async (fileIds: string[]): Promise<FileTocInfo[]> => {
  ...
  for (const file of files) {
    if (file.type === "structured_report" && file.structuredReportId) {
      ...
      if (toc) {
        filesTocInfo.push({
          fileId: file.id,
          fileName: file.metadata?.title ?? file.name ?? "Untitled",
          documentTitle: toc.metadata?.title ?? file.metadata?.title ?? file.name ?? "Untitled",
          documentSummary: toc.metadata?.summary ?? file.metadata?.summary ?? "",
...
export const fileAnswerAgent = async ({
  ...
  try {
    ...
    for (const info of filesTocInfo) {
          ...
          .map((section, idx) => {
            const subsections =
              section.subsections && section.subsections.length > 0
                ? section.subsections
                    .map((sub) => `    - ${sub.title} (pages ${sub.pageStart} - ${sub.pageEnd})`)
                    .join("\n")
                : "    - No subsections";
...
`;
    ...
    const getPageContentForFile = async (
      ...
      ((pages: number[]) => Promise<{ pageNumber: number; content: string }[]>) | null
    > => {
    ...
    if (pdfFiles.length > 0) {
      tools.fetchPagesFromFile = tool({
        ...
        inputSchema: z.object({
          ...
          query: z.string().describe("What information you are looking for in these pages"),
          pages: z.array(z.number()).describe("Array of page numbers to fetch from this document"),
        }),
        execute: async ({
          fileId,
          query: pageQuery,
          ...
          if (pageUsage) {
            mergeTokenUsage(usageRecord, MODELS.GEMINI_2_5_FLASH_LITE, pageUsage);
            if (addUsage) {
              addUsage({
                usage: pageUsage,
    ...
    if (pdfFiles.length > 0) {
      tools.similaritySearchChunks = tool({
        ...
        execute: async ({
          ...
          return chunks
            .map((chunk) => `Page ${chunk.pageNumber ?? "unknown"}: ${chunk.content}`)
            .join("\n\n");
        },
      });
    }
    ...
    const structuredReportFiles = filesTocInfo.filter((f) => f.isStructuredReport);
    if (structuredReportFiles.length > 0) {
      tools.getStructuredReportContent = tool({
        ...
        inputSchema: z.object({
          fileId: z.string().describe("ID of the structured report file to retrieve"),
        }),
...
export const getFileAnswerAgentTool = ({
  ...
  return tool({
    ...
    inputSchema: z.object({
      query: z
        .string()
        .describe("The question or information need to research across the documents"),
      fileIds: z
        .array(z.string())
        .describe("Array of file IDs to search within. Can include 'file_' prefix."),
    }),
    execute: async ({
      query,
      fileIds,
