...
export const snippetSearch = async ({
  query,
  limit = 25,
  userId,
  orgId,
}: { query: string; limit?: number; userId: string; orgId: string }) => {
...
export const getChunkSearchTool = ({ context }: { context: ToolContext }) => {
  return tool({
    ...
    inputSchema: z.object({
      query: z.string(),
      documentIds: z.array(z.string()).optional().describe("The document ids to search within"),
    }),
    execute: async ({
      query,
      documentIds,
...
export const getChapterSearchTool = ({ context }: { context: ToolContext }) => {
  return tool({
    ...
    execute: async ({ query }: { query: string }) => {
      return await similaritySearchChapters({
        query,
        limit: 3,
        userId: context.userId,
        orgId: context.orgId,
