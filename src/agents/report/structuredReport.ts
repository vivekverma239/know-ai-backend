...
export const processStructuredReport = async ({
    ...
    try {
        ...
        const tasks = subQuestionsOutput.subQuestions.map((q: any, index: number) =>
            limit(async () => {
                const existing = existingAnswers.find((ans: any) => ans.question === q.question);
                if (existing && !reprocess) return existing;

                const answer = modelConfig.subQuestionAnswerMethod === "similaritySearch"
