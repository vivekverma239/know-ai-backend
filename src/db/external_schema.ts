...
export const trends = createExternalTable("trends", (t) => ({
    id: bigint("id", { mode: "number" }).primaryKey(),
    documentId: bigint("document_id", { mode: "number" }),
    highlightId: bigint("highlight_id", { mode: "number" }).references(() => highlights.id, { onDelete: "cascade" }),
...
export const trendsAssets = createExternalTable("trends_assets", (t) => ({
    id: bigint("id", { mode: "number" }).primaryKey(),
    trendId: bigint("trend_id", { mode: "number" }).references(() => trends.id, { onDelete: "cascade" }),
...
export const scenarioRemarks = createExternalTable("scenario_remarks", (t) => ({
    id: bigint("id", { mode: "number" }).primaryKey(),
    scenarioId: bigint("scenario_id", { mode: "number" }).references(() => scenarios.id, { onDelete: "cascade" }),
    documentId: bigint("document_id", { mode: "number" }),
    entityId: bigint("entity_id", { mode: "number" }).references(() => entities.id),
...
// Join Tables
export const highlightsEntitiesRel = createExternalTable("highlights_entities_rel", (t) => ({
    highlightId: bigint("highlight_id", { mode: "number" }).references(() => highlights.id, { onDelete: "cascade" }),
    entityId: bigint("entity_id", { mode: "number" }).references(() => entities.id, { onDelete: "cascade" }),
    userDefined: boolean("user_defined").default(false),
}), (t) => ({
    pk: index("highlights_entities_rel_pk").on(t.highlightId, t.entityId),
}));

export const highlightsTagsRel = createExternalTable("highlights_tags_rel", (t) => ({
    highlightId: bigint("highlight_id", { mode: "number" }).references(() => highlights.id, { onDelete: "cascade" }),
...
export const calendarEventsEntitiesRel = createExternalTable("calendar_events_entities_rel", (t) => ({
    calendarEventId: bigint("calendar_event_id", { mode: "number" }).references(() => calendarEvents.id, { onDelete: "cascade" }),
    entityId: bigint("entity_id", { mode: "number" }).references(() => entities.id, { onDelete: "cascade" }),
}), (t) => ({
